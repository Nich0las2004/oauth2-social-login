...
public class FacebookControllerIntegrationTest {
    ...
    @Test
    @WithMockUser
    public void testSaveFacebookUserProfile() throws Exception {
        User user = new User();
        ...
        when(userService.saveUser(oAuth2User, "facebook")).thenReturn(user);

        mockMvc.perform(get("/facebook").with(oauth2Login().oauth2User(oAuth2User)))
                .andExpect(status().isOk())
                .andExpect(view().name("facebook"))
